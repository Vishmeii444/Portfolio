...
const Navbar = () => {
  ...
  const [isManualScrolling, setIsManualScrolling] = useState(false); // New lock state

  const handleClick = (id: string) => {
    setIsManualScrolling(true); // Lock the scroll listener
    setActiveId(id);

    const element = document.getElementById(id);
    element?.scrollIntoView({ behavior: "smooth" });

    // Unlock once the smooth scroll is likely finished (approx 800ms)
    setTimeout(() => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // IF we are manually clicking/scrolling, ignore the scroll position logic
            ...
            >
              ...
              {isActive && (
                <motion.div
                  ...
                  transition={{
                    // Switching to a 'tween' removes physics-based wobbling
                    type: "tween",
                    ease: [0.4, 0, 0.2, 1], // Standard "Exquisite" Cubic Bezier
                    duration: 0.5,
                  }}
                >
